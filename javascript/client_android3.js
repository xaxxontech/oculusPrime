...
function callServer(fn, str) {
	getFlashMovie("oculus_android3").flashCallServer(fn,str);
}
...
function play(str) {
	...
	getFlashMovie("oculus_android3").flashplay(num);
}
...
function setstatus(status, value) {
	if (value.toUpperCase() == "CONNECTED" && !connected) { // initialize
		...
		connected = true;
		messageAndroid("Connected to Oculus");
		//publish("camera");
	}
	if (status == "stream" && (value.toUpperCase() != streammode.toUpperCase())) {
		messageAndroid("stream: "+value);
		play(value); 
//		messageAndroid("(no error)");
	}
	...
	if (status == "user") { 
		username = value; 
		messageAndroid(username+ " signed in");
...
function loginsend(user, pass) {
	getFlashMovie("oculus_android3").connect(user+" "+pass+" false"); // false is for cookie param, not applicable here
}
